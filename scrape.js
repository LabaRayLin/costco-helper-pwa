...
async function scrape() {
    ...
    try {
        // 診斷模式：測試 pagination 欄位在確定的分類下是否合法
        const initialUrl = `https://www.costco.com.tw/rest/v2/taiwan/products/search?fields=products(code,name,summary,price(FULL),images(DEFAULT),stock(FULL),averageRating,variantOptions),pagination&query=:relevance:allCategories:hot-buys&pageSize=100&lang=zh_TW&curr=TWD&currentPage=0`;
        ...
        for (currentPage = 0; currentPage < pagesToFetch; currentPage++) {
            ...
            const url = `https://www.costco.com.tw/rest/v2/taiwan/products/search?fields=products(code,name,summary,price(FULL),images(DEFAULT),stock(FULL),averageRating,variantOptions),pagination&query=:relevance:allCategories:hot-buys&pageSize=100&lang=zh_TW&curr=TWD&currentPage=${currentPage}`;
